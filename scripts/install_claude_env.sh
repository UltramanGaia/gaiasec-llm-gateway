...
ANTHROPIC_AUTH_TOKEN_VALUE="${ANTHROPIC_AUTH_TOKEN_VALUE:-dummy}"
...
python3 - "$TEMPLATE_PATH" "$DEST_PATH" "$GATEWAY_URL" "$ANTHROPIC_AUTH_TOKEN_VALUE" "$MODEL_NAME" <<'PY'
import pathlib
import sys
...
content = content.replace("__ANTHROPIC_AUTH_TOKEN__", auth_token)
