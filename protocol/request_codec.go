...
func EncodeOpenAIChatRequest(ir IRRequest, modelName string) ([]byte, error) {
	req := openAIChatRequest{
		...
		ResponseFormat: cloneRaw(ir.ResponseFormat),
	...
	for _, msg := range msgs {
		...
		for _, rawPart := range content {
			...
			case "tool_result":
				...
				ir.Content = append(ir.Content, IRPart{Type: "tool_result", Output: stringValue(part["content"]), Text: stringValue(part["content"])})
			}
		}
		result = append(result, ir)
	}
...
func decodeGenericContent(content interface{}) []IRPart {
	...
	case []interface{}:
		...
		for _, rawPart := range c {
			...
			partType := normalizeIRPartType(firstNonEmpty(stringValue(part["type"]), "text"))
			irPart := IRPart{Type: partType, Text: stringValue(part["text"]), ProviderExtensions: map[string]interface{}{"raw": part}}
