# 2026-06-01 Gateway Responses-Upstream GPT-5.4 Smoke

## Scope

This report records a dedicated real-upstream smoke run for the gateway on `upstream_type=openai_responses` using model `gpt-5.4`.
...
```text
/tmp/gateway-responses-upstream-smoke-gpt54-20260601
```
...
This provides a fifth real model sample on the supported `openai_responses` route.

It shows that `gpt-5.4` currently behaves closer to `MiniMax/MiniMax-M2.5`, `gpt-5.4-mini`, and `gpt-5.5` than to `gpt-5.2` for `previous_response_id` follow-up scenarios.
